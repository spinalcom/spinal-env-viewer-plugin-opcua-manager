<template>
  ...
  >
    ...
    <md-dialog-actions>
      ...
      <md-button class="md-primary" @click="Link" :disabled="!profileSelected || state !== STATES.loaded">Link</md-button>
    </md-dialog-actions>
  </md-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    async getAllProfiles() {
      const profiles = await opcuaProfileService.getProfiles();
      const res = [];
    ...
    async Link() {
      try {
        ...
        this.state = this.STATES.loading;
        await opcuaProfileService.linkProfileToDevice(this.selectedNode, profileNode);
